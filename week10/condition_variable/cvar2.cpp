...
// Solution to the producer consumer problem using: Mutex + Global variable + Condition variable
...
void consumer() {
  int data = 0;
  while (true) {
    std::unique_lock<std::mutex> ul(g_mutex); // Acquire the lock on the same mutex
...
void producer() {
  while (true) {
    std::unique_lock<std::mutex> ul(g_mutex); // Aquire lock
    g_data = produceData(); 
    g_ready = true;
    ul.unlock(); 
    g_cv.notify_one();
